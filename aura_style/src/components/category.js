...
function Category() {
    ...
    const handleSubmit = async (id) => {
        try {
            const url = `${apiUrl}/deleteItemcat/${id}`;
            console.log("id", id);
    ...
    const handleAddCategory = async () => {
        ...
        try {
            const response = await axios.post(`${apiUrl}/addCategory`, { category: categoryName });
                ...
                <table style={{ width: '60%', borderCollapse: 'collapse' }}>
                    <thead>
                        <tr>
                            ...
                            <th style={{ backgroundColor: '#f2f2f2', padding: '12px 8px', textAlign: 'left' }}>Category Name</th>
                    ...
                    <tbody>
                        {list.map((value, index) => (
                            <tr key={index}>
                                ...
                                <td style={{ border: '5px solid rgba(255, 153, 153)', padding: '8px', borderColor: 'rgba(255, 153, 153)' }}>{value.category}</td>      
    ...
    <button style={{ backgroundColor: 'rgba(255, 153, 153)', color: '#000', padding: '12px 14px', cursor: 'pointer', borderRadius: '12%', borderColor: 'rgba(255, 153, 153)' }} onClick={() => handleSubmit(value._id)}>Delete</button>
            ...
            <div>
            <Button variant="outlined" onClick={handleClickOpen}  sx={{ bgcolor: 'rgba(255, 153, 153)', color: '#000',marginLeft:'530px',cursor: 'pointer', marginTop : "30px",borderColor: 'rgba(255, 153, 153)' }}>Add category</Button>
            <Dialog
                open={open}
                onClose={handleClose}
                PaperProps={{
